<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link href="https://stackpath.bootstrapcdn.com/bootswatch/4.1.1/solar/bootstrap.min.css" rel="stylesheet" >
    <link href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css" rel="stylesheet" >
    <link rel="stylesheet" href="css/pinterest.css">
    <link rel="stylesheet" href="css/carrusel.css">
    <link rel="stylesheet" href="css/footer.css">
    <title>TIENDA ARACELY</title>
</head>
<body>
<?php
include 'menu.php';
?>

<div class="container text-center">
    <h1>caja de galletas</h1>
    <div class="row">

        <div class="col-md-6">
            <div class="fondo-producto">
                <img src="imagenes/producto1.png" alt="">
            </div>

        </div>
        <div class="col-md-6">
            <div class="fondo-producto">
                <h2>caja de galletas</h2>
                <h3>Precio : s/.13.00</h3>

                <p>Lorum, nemo impedit sapiente ratione quae. Repellat nisi atque est, rerum ?</p>
                <p>
                    <a  class="btn btn-success" href="carrito.php">Comprar</a>
                    <a  class="btn btn-primary" href="index.php">Volver</a>
                </p>
            </div>
        </div>
    </div>

</div>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
<script src="js/pinterest_grid.js"></script>
<script src="js/main.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js"></script>
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.1/js/bootstrap.min.js" ></script>
</body>
</html>